...
-- Add additional security policies for sensitive tables
-- Ensure trees can only be accessed by authorized users
DROP POLICY IF EXISTS "Enhanced tree access control" ON public.trees;
CREATE POLICY "Enhanced tree access control"
ON public.trees
FOR SELECT
USING (
  auth.uid() = created_by OR 
  has_role(auth.uid(), 'admin'::app_role) OR 
  (has_role(auth.uid(), 'qtra_arborist'::app_role) AND EXISTS (
    SELECT 1 FROM assessments a 
    WHERE a.tree_id = trees.id 
    AND a.assessor_id = auth.uid()
  ))
);

-- Ensure assessments can only be accessed by authorized users  
DROP POLICY IF EXISTS "Enhanced assessment access control" ON public.assessments;
CREATE POLICY "Enhanced assessment access control"
...
USING (
  ...
  (has_role(auth.uid(), 'qtra_arborist'::app_role) AND EXISTS (
    ...
    WHERE t.id = assessments.tree_id
  ))
);
